...
from logic.logic_scanner.obras.logic_obrasKOMCA import scanner

class ScannerObrasKOMCA(ttk.Frame):
    def __init__(self, master, **kwargs):
        ...
        ttk.Label(frm, text="Archivo obras KOMCA (.xls): ").grid(row=0, column=0, sticky="e", padx=5, pady=5)
        ...
        self.tooltip = ImageTooltip(
            self.btn_help,
            image_path_getter=resource_path("assets", "info_obras", "komca", "img1.png"),
            text_getter="El PDF debe contener esta estructura para que se pueda procesar"
        )
    ...
    def elegir_pdf(self):
        dpath = filedialog.askopenfilename(
                title="Selecciona el archivo EXCEL",
                filetypes=[
                    ("Archivos Excel", "*.xlsx *.xlsm *.xltx *.xltm *.xls")
                ]
            )
        if dpath:
    ...
    def scanner(self):
        ...
        if not ruta_boletines:
            messagebox.showwarning("Atención", "Debe seleccionar un archivo EXCEL.")
            return
