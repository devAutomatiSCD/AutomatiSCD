...
from logic.logic_scanner.obras.logic_obrasAPRA import scanner

class ScannerObrasAPRA(ttk.Frame):
    def __init__(self, master, **kwargs):
        ...
        ttk.Label(frm, text="Archivo obras APRA (.txt): ").grid(row=0, column=0, sticky="e", padx=5, pady=5)
        ...
        self.tooltip = ImageTooltip(
            self.btn_help,
            image_path=resource_path("assets", "info_obras", "apra", "img1.png"),
            text="El archivo TXT en la carpeta deben contener esta estructura\n para que se pueda procesar"
        )
    ...
    def elegir_pdf(self):
        dpath = filedialog.askopenfilename(title="Selecciona el archivo TXT", filetypes=[("TXT files", "*.txt")])
        if dpath:
    ...
    def scanner(self):
        ...
        if not ruta_boletines:
            messagebox.showwarning("Atención", "Debe seleccionar un archivo TXT.")
            return
