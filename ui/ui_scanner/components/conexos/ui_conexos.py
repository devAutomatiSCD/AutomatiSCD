...
class ScannerConexos(ttk.Frame):
    def __init__(self, master, **kwargs):
        ...
        self.var_ruta_boletines = tk.StringVar()
        ...
        ttk.Label(frm, text="Carpeta conexos:").grid(row=0, column=0, sticky="e", padx=5, pady=5)
        ttk.Entry(frm, textvariable=self.var_ruta_boletines, width=70).grid(
            row=0, column=1, sticky="we", padx=5, pady=5
        )
        ttk.Button(frm, text="Elegir...", command=self.elegir_boletines).grid(row=0, column=2, padx=5, pady=5)
    ...
    def elegir_boletines(self):
        dpath = filedialog.askdirectory(title="Selecciona la carpeta")
        if dpath:
            self.var_ruta_boletines.set(dpath)

    def scanner(self):
        if self.trabajando:
            return

        ruta_boletines = self.var_ruta_boletines.get().strip()
        folio = self.var_folio.get().strip()

        if not ruta_boletines or not folio:
            messagebox.showerror("Error", "Falta carpeta o folio")
            return
        ...
        def worker():
            res = escanear_pdf_CM(ruta_boletines, folio, stop_event=self.stop_event)
